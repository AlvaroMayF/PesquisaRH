...
from src.services.idsecure_db_service import create_idsecure_user, update_idsecure_user, add_photo_to_idsecure, \
    trigger_idsecure_sync, check_user_exists, set_idsecure_user_status
...
def get_rh_data():
    ...
    try:
        ...
        cur_rh.execute(
            ...
            base_params)
        novas_contratacoes_30d = (cur_rh.fetchone() or {}).get('total', 0)
        cur_rh.execute(
            ...
            base_params)
        media_anos_result = cur_rh.fetchone()
        ...
        cur_rh.execute(f"SELECT COUNT(*) as total FROM colaboradores WHERE {table_where_sql}", table_params)
        total_items = (cur_rh.fetchone() or {}).get('total', 0)
        total_pages = math.ceil(total_items / ITEMS_PER_PAGE) if total_items > 0 else 1
        offset = (page - 1) * ITEMS_PER_PAGE
        cur_rh.execute(
            f"SELECT id, nome, cpf, setor, cargo, unidade, ativo, foto_filename FROM colaboradores WHERE {table_where_sql} ORDER BY nome ASC LIMIT %s OFFSET %s",
            table_params + [ITEMS_PER_PAGE, offset])
        colaboradores_filtrados = cur_rh.fetchall()
